...
class UnsupNER:
    ...
    def tag_sentence(self, sent, rfp, dfp):
        ...
        masked_sent_arr, span_arr = self.filter_common_noun_spans(
            span_arr, masked_sent_arr, terms_arr
        )
        singleton_sentences, singleton_spans_arr = self.gen_single_phrase_sentences(
            terms_arr, masked_sent_arr, span_arr, rfp, dfp
        )
        singleton_entities = self.find_singleton_entities(
            singleton_sentences, singleton_spans_arr
        )
        self.find_entities(
            sent, terms_arr, masked_sent_arr, span_arr, singleton_entities, rfp, dfp
        )
        print("--------")
    ...
    def tag_se_in_sentence(self, sent, rfp, dfp):
        ...
        detected_entities_arr = self.find_entities(
            sent, terms_arr, masked_sent_arr, span_arr, singleton_entities, rfp, dfp
        )
    ...
    def gen_single_phrase_sentences(
        self, terms_arr, masked_sent_arr, span_arr, rfp, dfp
    ):
    ...
    def find_entities(
        self, sent, terms_arr, masked_sent_arr, span_arr, singleton_entities, rfp, dfp
    ):
        # print(sent)
        print(span_arr)
        dfp.write(sent + "\n")
        ...
        self.emit_sentence_entities(
            sent, terms_arr, detected_entities_arr, span_arr, rfp
        )
        dfp.flush()
        return detected_entities_arr
    ...
    def fill_detected_entities(self, detected_entities_arr, entities, pan_arr):
        ...
        final_sorted_d = OrderedDict(
            sorted(entities_dict.items(), key=lambda kv: kv[1], reverse=True)
        )
        first = "OTHER"
    ...
    def resolve_entities(self, index, singleton_entities, detected_entities_arr):
        if singleton_entities[index] != detected_entities_arr[index]:
            ...
            detected_entities_arr[index] = (
                TYPE2_AMB
                + detected_entities_arr[index]
                + "/"
                + singleton_entities[index]
            )

    def emit_sentence_entities(
        self, sent, terms_arr, detected_entities_arr, span_arr, rfp
    ):
        print("Final result")
    ...
    with open(file_name) as fp:
        ...
        for line in fp:
            if len(line) > 1:
                ...
                print(
                    "*******************:",
                    terms_arr[span_arr.index(1)][WORD_POS].rstrip(":"),
                    entity_arr[0],
                )
                sfp.write(
                    terms_arr[span_arr.index(1)][WORD_POS].rstrip(":")
...
def test_canned_sentences(obj):
    ...
    obj.tag_sentence(
        "He felt New York has a chance to win this year's competition", rfp, dfp
    )
    obj.tag_sentence("He flew from New York to SFO", rfp, dfp)
    obj.tag_sentence(
    ...
    obj.tag_sentence(
        "Fyodor Mikhailovich Dostoevsky was treated for Parkinsons", rfp, dfp
    )
    ...
    obj.tag_sentence(
        "Elon Musk said Telsa will open a manufacturing plant in Europe", rfp, dfp
    )
    obj.tag_sentence("He flew from New York to SFO", rfp, dfp)
    obj.tag_sentence(
    ...
    obj.tag_sentence(
        "Everyday he rode his bicycle from Rajakilpakkam to Tambaram", rfp, dfp
    )
    obj.tag_sentence(
